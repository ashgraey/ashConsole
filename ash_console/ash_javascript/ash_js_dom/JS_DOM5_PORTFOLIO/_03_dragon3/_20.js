...
// -------------- Enemy ----------------------
// init setting 0. EnemySpot 위치 잡기, setting
...
// a == player, b == enenmy
// collision => returnt True
function collision(a, b) {
  // 작은 사각형을 기준으로 큰 사각형의 모서리의 collison을 감지하는것이 쉽다.
  // 단순히 가로가 큰걸로 교체해주는 것
  if (a["w"] > b["w"]) {
    temp = a;
    a = b;
...
// bolt객체와 Enemy객체가 충돌시 객체를 삭제시킴
// collision에 연결되어있음
// 충돌시 splice
// return은 break같은 역할 => 충돌 발생 시 빠르게 함수를 빠져나옴
...
//================ bg ===============================
//--------------- draw ------------------------------
// update 실행에 필요한 구조적 로직을 먼저 실행시킨다.
function update() {
  moveBg();
  movePlayer();
  moveBoltList();
  ...
  shootPlayer();
  // 게임이 실행중일에만 enemy 이동(dead 기능도 포함) 및
  if (gameRun) {
    moveEnmeyList();
    setEnmeyTimer(300);
  }
}
// update 이후 출력에 관련된 부분의 함수를 실행시킴
// 정리 기능 => 출력순
function draw() {
