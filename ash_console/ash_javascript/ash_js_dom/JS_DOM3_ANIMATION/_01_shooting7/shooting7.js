...
function setSpotList(size) {
  for (var i = 0; i < size; i++) {
    spotList[i] = { x: 45 + i * 100, y: 50, size: 10, color: "palegreen" };
  }
}
...
function collison(enemy) {
  ...
  else if (inRect(player["x"] + player["size"], player["y"], enemy))
    return true;
  // player의 왼쪽 하단 모서리가 닿으면
  else if (inRect(player["x"], player["y"] + player["size"], enemy))
    return true;
  // player의 오른쪽 하단 모서리가 닿으면
  else if (
    inRect(player["x"] + player["size"], player["y"] + player["size"], enemy)
  )
    return true;
  else return false;
}
...
init();
setInterval(draw, 10); // 1초에 100번 동작
