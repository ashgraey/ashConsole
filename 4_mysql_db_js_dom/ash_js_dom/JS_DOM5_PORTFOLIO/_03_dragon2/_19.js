...
function moveBoltList() {
  for (var i = 0; i < boltList.length; i++) {
    ...
    if (boltList[i]["dead"] <= 0) {
      // 조건식 true : bolt 삭제, return
      // boltList 첫번째 bolt 삭제
      boltList.splice(i, 1);
      return;
    }
    // 조건식 false : bolt의 움직임 구현
...
function createBolt() {
  ...
  // len의 index 값으로 사용
  len = boltList.length;
  // boltList에 bolt 객체 담기
  boltList[len] = bolt;
  console.log(boltList);
}
// 슈팅 조절
// keyDown => shootKey["up"] = "shoot"
// shootPlayer => shootKey["up"] = "wait"
// keyUp => shootKey["up"] = "ready"
